...
<script>
    ...
    document.addEventListener('realtime:beforeUpdate', (e) => {
        if (e.detail.el.id === 'table-body') {
            Array.from(e.detail.newEl.querySelectorAll('tr.record-row')).forEach(tr => {
...
<div id="disputes-table-card" class="rounded-xl border border-gray-300 bg-white shadow-sm mt-4 overflow-hidden <?= $currentTab !== 'disputes' ? 'hidden' : '' ?>">
    <div class="overflow-x-auto">
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100" id="disputes-table-body">
                ...
                <?php else: ?>
                    ...
                    foreach ($disputes as $d): 
                        $initialDisplay = ($dispIndex >= 8) ? 'style="display: none;"' : '';
                    ?>
                        <tr class="hover:bg-gray-50 transition-colors dispute-row" <?= $initialDisplay ?>>
                            <?php $dispIndex++; ?>
                            ...
                            <td class="py-3 px-4 max-w-[200px]">
                                <div class="truncate text-xs font-medium text-amber-800 bg-amber-50 px-2 py-1 rounded block w-full" title="<?= htmlspecialchars($d['description']) ?>">
                                ...
                                <?php if ($currStatus === 'Pending RadTech Review'): ?>
                                    <div class="flex items-center justify-start gap-1">
                                        <button type="button" onclick="confirmReupload(<?= $d['case_id'] ?>)"
                                                class="text-blue-500 hover:text-blue-700 transition" title="Re-upload & Correct">
                                        ...
                                        <button type="button" onclick="openEscalateModal(<?= $d['id'] ?>, '<?= htmlspecialchars($d['case_number']) ?>')"
                                                class="text-red-500 hover:text-red-700 transition" title="Escalate to Radiologist">
                                        ...
                                        <button type="button" onclick="openResolveModal(<?= $d['id'] ?>, '<?= htmlspecialchars($d['case_number']) ?>', 'Fix & Resolve Ticket', <?= htmlspecialchars(json_encode($d['description']), ENT_QUOTES, 'UTF-8') ?>)"
                                                class="text-green-500 hover:text-green-700 transition" title="Fix & Resolve">
    ...
    <div class="px-4 py-3 border-t border-gray-200 bg-gray-50 flex items-center justify-between rounded-b-lg">
        <div class="text-sm text-gray-700">
            Showing <span id="disp-page-start" class="font-medium">0</span> to <span id="disp-page-end" class="font-medium">0</span> of <span id="disp-page-total" class="font-medium">0</span> results
        </div>
        <div class="flex items-center gap-2">
            <button type="button" id="disp-btn-prev" onclick="changeDispPage(-1)" class="px-3 py-1 text-sm border border-gray-300 rounded-md bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">Previous</button>
            <div id="disp-page-numbers" class="flex items-center gap-1"></div>
            <button type="button" id="disp-btn-next" onclick="changeDispPage(1)" class="px-3 py-1 text-sm border border-gray-300 rounded-md bg-white text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">Next</button>
        </div>
    </div>
</div>

<script>
    let dispCurrentPage = 1;
    const dispPageSize = 8;
    
    function renderDispPagination() {
        const rows = Array.from(document.querySelectorAll('#disputes-table-body tr.dispute-row'));
        const total = rows.length;
        const totalPages = Math.ceil(total / dispPageSize) || 1;
        
        if (dispCurrentPage > totalPages) dispCurrentPage = totalPages;
        if (dispCurrentPage < 1) dispCurrentPage = 1;

        const startIdx = (dispCurrentPage - 1) * dispPageSize;
        const endIdx = Math.min(startIdx + dispPageSize, total);
        ...
        document.getElementById('disp-page-total').textContent = total;
        document.getElementById('disp-page-start').textContent = total === 0 ? 0 : startIdx + 1;
        document.getElementById('disp-page-end').textContent = endIdx;

        document.getElementById('disp-btn-prev').disabled = dispCurrentPage === 1;
        document.getElementById('disp-btn-next').disabled = dispCurrentPage === totalPages;

        const pageNumbers = document.getElementById('disp-page-numbers');
        pageNumbers.innerHTML = '';
        
        let startPage = Math.max(1, dispCurrentPage - 2);
        let endPage = Math.min(totalPages, startPage + 4);
        if (endPage - startPage < 4) {
            startPage = Math.max(1, endPage - 4);
        }
        ...
        for (let i = startPage; i <= endPage; i++) {
            const btn = document.createElement('button');
            btn.type = 'button';
            btn.className = `px-3 py-1 text-sm border rounded-md ${i === dispCurrentPage ? 'bg-red-600 text-white border-red-600' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`;
            btn.textContent = i;
            btn.onclick = () => {
                dispCurrentPage = i;
                renderDispPagination();
            };
            pageNumbers.appendChild(btn);
        }
    }

    function changeDispPage(delta) {
        dispCurrentPage += delta;
        renderDispPagination();
    }

    document.addEventListener('DOMContentLoaded', () => {
        setTimeout(renderDispPagination, 100);
    });
</script>
