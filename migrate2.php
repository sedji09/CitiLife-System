...
// Extract disputes table
$startTable = strpos($sourceContent, '<!-- DISPUTES / ERROR REPORTS TABLE CARD (Hidden by default) -->');
if ($startTable === false) die("Cannot find disputes table in source.");
...
// Replace "Pending RadTech Verification" text to "Under verification"
...
$tableHTML = str_replace(
    ...
    'class="<?= (($_GET[\'tab\'] ?? \'\') === \'disputes\') ? \'\' : \'hidden\' ?> rounded-xl',
    $tableHTML
);

// Clean up JS (remove switchRadtechTab and DOMContentLoaded)
