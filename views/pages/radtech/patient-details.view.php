    ...
    <?php if ($isReadOnly && $userRole === 'radtech' && !($isAmendMode ?? false)): ?>
        <?php if ($caseDetails['status'] === 'Report Ready'): ?>
            <div class="mt-5 rounded-lg bg-purple-50 border border-purple-300 p-4 flex items-center gap-3 shadow-xs">
                ...
                <p class="text-sm text-purple-800 font-medium">The radiologist report is ready. You can review the findings
                    below and print the result.</p>
            </div>
        ...
        <div class="rounded-xl border border-gray-300 bg-white p-6 shadow-sm">
            ...
            <div class="rounded-lg bg-red-50 border border-red-200 p-4">
                ...
                <div class="px-2 space-y-2">
                    ...
                    <div id="pv_patient_philhealth_container"
                        class="flex justify-between text-sm <?= !empty($caseDetails['philhealth_id']) ? '' : 'hidden' ?>">
                        <span class="text-gray-600">PhilHealth Number</span>
                        <span id="pv_patient_philhealth"
        ...
        <div class="rounded-xl border border-gray-300 bg-white p-6 shadow-sm">
            ...
            <div class="space-y-4">
                ...
                <div class="pt-1">
                    ...
                    ?>
                    <span id="case-status-badge"
                        class="inline-block font-bold text-xs px-3 py-1.5 rounded-full transition-all duration-300"
    ...
    <?php if (!empty($activeDispute)): ?>
        ...
        <div class="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch">
            ...
            <div class="rounded-xl border border-gray-300 bg-white p-6 shadow-sm flex flex-col h-full space-y-4">
                <div>
                    <div class="flex items-center justify-between mb-1">
                        ...
                        <span
                            class="text-xs font-semibold text-gray-500 bg-gray-100 border border-gray-200 rounded-full px-2.5 py-0.5">
                            <?= $existingCount = count(json_decode($caseDetails['image_path'] ?? '[]', true) ?: [$caseDetails['image_path'] ?? '']) ?>
                            <?= $existingCount === 1 ? 'file' : 'files' ?>
                        </span>
                    </div>
            ...
            <div class="rounded-xl border border-amber-300 bg-white shadow-sm flex flex-col h-full overflow-hidden">
                <!-- Header -->
                <div
                    class="flex items-center justify-between gap-2 px-5 py-2.5 bg-gradient-to-r from-amber-50 to-orange-50 border-b border-amber-100">
                    <div class="flex items-center gap-2">
                    ...
                    <?php if ($isEdited): ?>
                        <span
                            class="text-xs font-bold text-emerald-700 bg-emerald-50 border border-emerald-300 px-3 py-1 rounded-full flex items-center gap-1.5 shadow-2xs shrink-0">
                    ...
                    <?php if ($showXrayTemplateName): ?>
                        <div class="space-y-4">
                            <?php if ($reqCorrectTemplate || $reqNotes || $reqSide): ?>
                                ...
                                <div class="rounded-xl bg-gray-50/80 border border-gray-300 p-4 px-5 py-4 text-xs text-black space-y-2"
                                    style="padding: 1rem 1.25rem;">
                                    <div class="flex items-center justify-between font-semibold text-black">
                                        <span>Patient Requested Change:</span>
                                        <?php if ($reqSide): ?>
                                            <span
                                                class="px-2 py-0.5 rounded bg-white text-gray-800 font-semibold text-[11px] border border-gray-300 uppercase tracking-wider">Side:
                                                <?= htmlspecialchars($reqSide) ?></span>
                                        <?php endif; ?>
                                    </div>
                                    <?php if ($reqCorrectTemplate): ?>
                                        <p class="text-black font-normal">Desired Template: <strong
                                                class="font-bold text-black"><?= htmlspecialchars($reqCorrectTemplate) ?></strong></p>
                                    <?php endif; ?>
                                    <?php if ($reqNotes): ?>
                            ...
                            <div class="space-y-2">
                                <div class="flex items-center justify-between">
                                    <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider">X-RAY TEMPLATE
                                        / EXAM NAME</label>
                                    <?php if (!$isEdited): ?>
                                        <div class="flex items-center gap-1.5">
                                            <span class="text-xs text-gray-400 font-medium mr-0.5">Quick Side:</span>
                                            <button type="button" onclick="applyRadtechSidePrefix('Left')"
                                                class="px-2.5 py-1 text-xs font-semibold bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-md border border-gray-200 transition cursor-pointer active:scale-95">Left</button>
                                            <button type="button" onclick="applyRadtechSidePrefix('Right')"
                                                class="px-2.5 py-1 text-xs font-semibold bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-md border border-gray-200 transition cursor-pointer active:scale-95">Right</button>
                                            <button type="button" onclick="applyRadtechSidePrefix('Bilateral')"
                                                class="px-2.5 py-1 text-xs font-semibold bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-md border border-gray-200 transition cursor-pointer active:scale-95">Bilateral</button>
                                        </div>
                                    <?php endif; ?>
                                </div>
                                <div class="relative">
                                    <input type="text" id="amend_exam_type_input" name="amend_exam_type"
                                        value="<?= htmlspecialchars($reqCorrectTemplate ?: ($caseDetails['exam_type'] ?? '')) ?>"
                                        <?= $isEdited ? 'readonly disabled' : '' ?> placeholder="e.g. Left Knee AP / Lateral"
                                ...
                                <div class="text-xs text-gray-500 flex items-center gap-1.5 pt-0.5">
                                    ...
                                    <strong
                                        class="font-bold text-black"><?= htmlspecialchars($caseDetails['exam_type'] ?? '') ?></strong>
                                </div>
                            </div>
                        </div>
                    <?php endif; ?>
        ...
        <div class="<?= $isReadOnly ? 'mt-8 grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch' : 'mt-8' ?>">
            <div class="rounded-xl border border-gray-300 bg-white p-6 shadow-sm flex flex-col h-full">
                ...
                <div id="file-size-error" style="display:none;"
                    ...
                    <p id="file-size-error-msg" class="text-sm text-red-700 font-medium">File exceeds the 15 MB maximum
                        size.</p>
                </div>
                ...
                <div id="no-image-error" style="display:none;"
                    ...
                    <p class="text-sm text-red-700 font-medium">Please upload at least one diagnostic image before
                        submitting.</p>
                </div>
                ...
                <div id="exam-required-error" style="display:none;"
                    ...
                    <p class="text-sm text-amber-700 font-medium">Please select Examination Types above before uploading
                        images.</p>
                </div>
                ...
                <div id="limit-error" style="display:none;"
                    ...
                    <p id="limit-error-msg" class="text-sm text-orange-700 font-medium">You can only upload as many images
                        as there are selected exams.</p>
                </div>
                    ...
                    <div style="display:grid; grid-template-columns:1fr 1fr; gap:1.5rem; align-items:start;">
                        ...
                        <div id="drop-zone"
                            ...
                            <div class="text-center p-4 pointer-events-none">
                                ...
                                <p class="text-xs text-gray-400">Patient: <?= htmlspecialchars(formatFullName($caseDetails)) ?>
                                </p>
                                <p class="text-xs text-gray-400 mt-1">Max <strong class="text-gray-500">15 MB</strong> per file
                                </p>
                            </div>
                        ...
                        <div id="file-preview-area"
                            ...
                            <p id="no-file-msg" style="font-size:0.875rem;color:#9ca3af;font-style:italic;">No files selected
                                yet.</p>
                        </div>
                    </div>
                <?php else: ?>
                        ...
                        <div class="flex flex-wrap gap-4">
                            <?php foreach ($savedPaths as $idx => $sPath): ?>
                                ...
                                <div onclick="openXrayLightbox('<?= PROJECT_DIR ? '/' . PROJECT_DIR . '/' : '/' ?><?= htmlspecialchars($sPath) ?>')"
                                    ...
                                    title="<?= htmlspecialchars($imgLabel) ?> — Click to view fullscreen">
                                    <img src="<?= PROJECT_DIR ? '/' . PROJECT_DIR . '/' : '/' ?><?= htmlspecialchars($sPath) ?>"
                                        alt="<?= htmlspecialchars($imgLabel) ?>" style="width: 100%; height: 100%; object-fit: contain;"
                                        class="opacity-90 group-hover:opacity-100 group-hover:scale-105 transition-all duration-200">
                    ...
                    <div class="rounded-xl border border-gray-300 bg-white p-6 shadow-sm flex flex-col h-full">
                        <div class="mb-4 flex items-center gap-2">
                            <i data-lucide="file-text"
                                class="h-5 w-5 <?= $isReportReady ? 'text-red-500' : 'text-gray-400' ?>"></i>
                            <h3 class="text-lg font-semibold <?= $isReportReady ? 'text-gray-800' : 'text-gray-500' ?>">
                                Radiologist Report Findings</h3>
                        </div>
                                ...
                                <?php if ($isMultiExam): ?>
                                    <?php foreach ($parsedFindings as $examName => $reportData): ?>
                                        <div class="mb-4 last:mb-0 border-b border-gray-200 pb-3 last:border-0 last:pb-0">
                                            <h5
                                                class="text-xs font-bold text-red-600 mb-2 uppercase tracking-wide flex items-center gap-1.5">
                        ...
                        <?php else: ?>
                            ...
                            <div
                                ...
                                <p class="text-xs text-gray-500 max-w-[280px]">The radiologist has not yet submitted the findings
                                    and impression for this case.</p>
                            </div>
                        <?php endif; ?>
                ...
                <?php if ($isReportReady): ?>
                    <?php if (empty($caseDetails['released']) || (int) $caseDetails['released'] === 0): ?>
                        <button type="button" onclick="triggerReEdit(<?= $caseId ?>, this, event)"
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-yellow-600" viewBox="0 0 24 24" fill="none"
...
<div id="edit-patient-info-modal"
    class="fixed inset-0 z-[9999] hidden bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
    <div
        class="bg-white rounded-2xl shadow-2xl max-w-xl w-full border border-gray-100 my-8 overflow-hidden transform transition-all">
        <!-- Modal Header -->
        <div
            class="px-6 py-4 bg-gradient-to-r from-blue-50 to-indigo-50 border-b border-gray-200 flex items-center justify-between">
            <div class="flex items-center gap-3">
                <div
                    class="w-10 h-10 rounded-xl bg-blue-600 text-white flex items-center justify-center shadow-md shadow-blue-500/20">
            ...
            <button type="button" onclick="closeEditPatientInfoModal()"
                class="w-8 h-8 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-white/80 transition flex items-center justify-center cursor-pointer">
        ...
        <form method="POST" action="" id="edit-patient-info-form" onsubmit="return submitEditPatientInfo(event);"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                    <label class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1">First Name <span
                ...
                <div>
                    <label class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1">Last Name <span
                            class="text-red-500">*</span></label>
            ...
            <div>
                <label class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1">Middle Name <span
                        class="text-gray-400 font-normal text-[11px]">(Optional)</span></label>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                    <div class="flex items-center justify-between mb-1">
                        <label class="block text-xs font-bold text-gray-700 uppercase tracking-wider">Birthdate <span
                                class="text-red-500">*</span></label>
                        <span id="modal_pat_calc_age"
                    ...
                    <div class="relative">
                        ...
                        <i data-lucide="calendar"
                            class="absolute right-3 top-3 w-4 h-4 text-gray-400 pointer-events-none"></i>
                    </div>
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1">Sex <span
                            class="text-red-500">*</span></label>
                    <select name="sex" id="modal_pat_sex" required
                        class="w-full text-sm text-gray-900 bg-gray-50 border border-gray-200 focus:bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-100 rounded-xl p-2.5 outline-none transition">
                        <option value="Male" <?= ($caseDetails['sex'] ?? '') === 'Male' ? 'selected' : '' ?>>Male</option>
                        <option value="Female" <?= ($caseDetails['sex'] ?? '') === 'Female' ? 'selected' : '' ?>>Female
                        </option>
                    </select>
                </div>
            </div>
            ...
            <!-- Contact Number -->
            <div>
                <label class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1">Contact Number <span
                        class="text-red-500">*</span></label>
                <input type="tel" name="contact_number" id="modal_pat_contact" required maxlength="11"
                    pattern="09[0-9]{9}" value="<?= htmlspecialchars($caseDetails['contact_number'] ?? '') ?>"
                    placeholder="09XXXXXXXXX" oninput="this.value = this.value.replace(/[^0-9]/g, '').slice(0, 11);"
                    class="w-full text-sm text-gray-900 bg-gray-50 border border-gray-200 focus:bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-100 rounded-xl p-2.5 outline-none transition">
            </div>
            ...
            <!-- PhilHealth Status (Read-Only: Assigned during Assign Exam) -->
            <div>
                <div class="flex items-center justify-between mb-1">
                    <label class="block text-xs font-bold text-gray-700 uppercase tracking-wider">PhilHealth
                        Status</label>
                    <span class="text-[11px] font-medium text-gray-400 flex items-center gap-1">
                        <i data-lucide="lock" class="w-3 h-3 text-gray-400"></i> Assigned in Assign Exam
                    </span>
                </div>
                <div
                    class="w-full text-sm text-gray-700 bg-gray-100/90 border border-gray-200 rounded-xl p-2.5 flex items-center justify-between select-none">
                    <span
                        class="font-semibold text-gray-800"><?= htmlspecialchars($caseDetails['philhealth_status'] ?? 'Without PhilHealth Card') ?></span>
                    <span
                        class="text-xs px-2.5 py-0.5 rounded-full font-semibold <?= ($caseDetails['philhealth_status'] ?? '') === 'With PhilHealth Card' ? 'bg-blue-100 text-blue-700 border border-blue-200' : 'bg-gray-200 text-gray-600' ?>">
                        <?= ($caseDetails['philhealth_status'] ?? '') === 'With PhilHealth Card' ? 'With PhilHealth Card' : 'Without PhilHealth Card' ?>
                    </span>
                </div>
                <input type="hidden" name="philhealth_status" id="modal_pat_philhealth_status"
                    value="<?= htmlspecialchars($caseDetails['philhealth_status'] ?? 'Without PhilHealth Card') ?>">
            </div>

            <?php if (($caseDetails['philhealth_status'] ?? '') === 'With PhilHealth Card'): ?>
                <!-- PhilHealth ID Details (Read-Only) -->
                <div id="modal_pat_philhealth_container"
                    class="p-3.5 bg-blue-50/70 border border-blue-100 rounded-xl space-y-2.5">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                            <label
                                class="block text-[11px] font-bold text-blue-900 uppercase tracking-wider mb-1">PhilHealth
                                ID Number</label>
                            <div
                                class="w-full text-sm font-mono font-medium text-blue-950 bg-white/90 border border-blue-200 rounded-xl p-2.5 select-none">
                                <?= htmlspecialchars($caseDetails['philhealth_id'] ?? '—') ?>
                            </div>
                            <input type="hidden" name="philhealth_id" id="modal_pat_philhealth_id"
                                value="<?= htmlspecialchars($caseDetails['philhealth_id'] ?? '') ?>">
                        </div>
                        <div>
                            <label class="block text-[11px] font-bold text-blue-900 uppercase tracking-wider mb-1">Patient's
                                Relation to ID</label>
                            <div
                                class="w-full text-sm font-medium text-blue-950 bg-white/90 border border-blue-200 rounded-xl p-2.5 select-none">
                                <?= htmlspecialchars($caseDetails['philhealth_relation'] ?? '—') ?>
                            </div>
                            <input type="hidden" name="philhealth_relation" id="modal_pat_philhealth_relation"
                                value="<?= htmlspecialchars($caseDetails['philhealth_relation'] ?? '') ?>">
                        </div>
                    </div>
                </div>
            <?php else: ?>
                <input type="hidden" name="philhealth_id" id="modal_pat_philhealth_id" value="">
                <input type="hidden" name="philhealth_relation" id="modal_pat_philhealth_relation" value="">
            <?php endif; ?>
...
<script>
    ...
    function togglePhilHealthModalFields() {
        // PhilHealth is read-only in this modal as it was assigned during Assign Exam
    }
    ...
    function updateModalPatRelationSelect(ownerUsed = false, ownerDate = '', familyUsed = false, familyDate = '') {
        ...
        relSelect.innerHTML = `
            ...
            <option value="Principal Member" id="pat-modal-opt-owner" ${ownerUsed ? 'disabled' : ''} ${targetVal === 'Principal Member' ? 'selected' : ''}>${ownerLabel}</option>
            <option value="Qualified Dependent" id="pat-modal-opt-family" ${familyUsed ? 'disabled' : ''} ${targetVal === 'Qualified Dependent' ? 'selected' : ''}>${familyLabel}</option>
        `;
    ...
    function checkPatPhilHealthModalDup() {
        ...
        if (digits.length < 12) {
            ...
            updateModalPatRelationSelect(false, '', false, '');
        ...
        patPhilHealthCheckTimeout = setTimeout(() => {
            ...
            fetch(url, { cache: 'no-store' })
                ...
                .then(data => {
                    ...
                    updateModalPatRelationSelect(ownerUsed, ownerDate, familyUsed, familyDate);

                    if (ownerUsed && familyUsed) {
                        idInput.setCustomValidity("This PhilHealth ID is already fully utilized.");
                        if (msgEl) {
                            msgEl.className = 'text-xs text-red-600 mt-1.5 block font-medium';
                        ...
                        if (msgEl) {
                            msgEl.className = 'text-xs text-red-600 mt-1.5 block font-medium';
                            msgEl.innerHTML = `Principal Member was already used on <strong>${ownerDate}</strong>${data.owner_patient_name ? ' (' + data.owner_patient_name + ')' : ''}. Only <strong>Qualified Dependent</strong> is available.`;
                            msgEl.classList.remove('hidden');
                        }
                    } else if (familyUsed) {
                        if (msgEl) {
                            msgEl.className = 'text-xs text-red-600 mt-1.5 block font-medium';
                            msgEl.innerHTML = `Qualified Dependent was already used on <strong>${familyDate}</strong>${data.family_patient_name ? ' (' + data.family_patient_name + ')' : ''}. Only <strong>Principal Member</strong> is available.`;
                            msgEl.classList.remove('hidden');
                        }
                    } else {
                        if (msgEl) {
                            msgEl.className = 'text-xs text-emerald-600 mt-1.5 block font-medium';
    ...
    async function submitEditPatientInfo(e) {
        ...
        try {
            ...
            if (result.success) {
                // Instantly update the Patient Verification card on the page
                if (result.patient) {
                    const pat = result.patient;
                    const nameEl = document.getElementById('pv_patient_full_name');
                    if (nameEl) nameEl.textContent = pat.full_name;

                    const ageSexEl = document.getElementById('pv_patient_age_sex');
                    if (ageSexEl) ageSexEl.textContent = (pat.age ? pat.age : '') + (pat.age && pat.sex ? ' / ' : '') + pat.sex;

                    const contactEl = document.getElementById('pv_patient_contact');
                    if (contactEl) contactEl.textContent = pat.contact_number || '—';

                    const addressEl = document.getElementById('pv_patient_address');
                    if (addressEl) addressEl.textContent = pat.home_address || '—';

                    const philhealthContainer = document.getElementById('pv_patient_philhealth_container');
                    const philhealthEl = document.getElementById('pv_patient_philhealth');
                    if (pat.philhealth_id) {
                        if (philhealthEl) {
                            philhealthEl.textContent = pat.philhealth_id + (pat.philhealth_relation ? ` (${pat.philhealth_relation})` : '');
                        }
                        if (philhealthContainer) philhealthContainer.classList.remove('hidden');
                    } else {
                        if (philhealthContainer) philhealthContainer.classList.add('hidden');
                    }
                }

                closeEditPatientInfoModal();
...
<div id="xray-lightbox-modal"
    class="fixed inset-0 z-[9999] hidden bg-black/80 backdrop-blur-sm flex flex-col items-center justify-center p-4 opacity-0 transition-opacity duration-300">
    <div class="relative inline-block max-w-[95vw] max-h-[90vh]">
        <!-- Close Button -->
        <button type="button" onclick="closeXrayLightbox()"
            class="absolute text-black bg-white hover:bg-gray-200 rounded-full p-2 transition-colors z-10 shadow-lg border border-gray-300"
            style="top: -16px; right: -16px;" title="Close">
            <i data-lucide="x" class="w-5 h-5"></i>
        </button>
        <!-- The Image -->
        <img id="xray-lightbox-main-img" src=""
            class="max-w-[95vw] max-h-[90vh] object-contain rounded-lg shadow-2xl scale-95 transition-transform duration-300 bg-black">
    </div>
</div>
...
<div id="release-loading-overlay"
    ...
    <div
        class="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-2xl flex flex-col items-center max-w-xs w-full mx-4 border border-gray-100 dark:border-gray-700 transition-all">
