...
if (!empty($case['pdf_path'])) {
    ...
    if (file_exists($pdfAbsPath)) {
        if ($isDownload) {
            ...
            header('Content-Disposition: attachment; filename="'.basename($case['pdf_path']).'"');
            header('Expires: 0');
            header('Cache-Control: must-revalidate');
            header('Pragma: public');
            header('Content-Length: ' . filesize($pdfAbsPath));
            readfile($pdfAbsPath);
            exit;
        } else {
            // Output PDF to browser for viewing/printing
            header('Content-Type: application/pdf');
            header('Content-Disposition: inline; filename="' . basename($case['pdf_path']) . '"');
...
if (!$isMultiExam && !empty($findingsRaw) && preg_match('/(?:^|\n+)\[([^\]]+)\]/', $findingsRaw)) {
    ...
    if (preg_match_all($pattern, $findingsRaw, $mF, PREG_OFFSET_CAPTURE)) {
        $mI = [];
        if (!empty($impressionRaw)) {
            preg_match_all($pattern, $impressionRaw, $mI, PREG_OFFSET_CAPTURE);
        }
        $countF = count($mF[0]);
        for ($i = 0; $i < $countF; $i++) {
            $eName = trim($mF[1][$i][0]);
            ...
            if (!empty($mI[0])) {
                ...
                for ($j = 0; $j < $countI; $j++) {
                    if (strcasecmp(trim($mI[1][$j][0]), $eName) === 0) {
                        ...
                        $endI = ($j + 1 < $countI) ? $mI[0][$j + 1][1] : strlen($impressionRaw);
                        $iBody = trim(substr($impressionRaw, $startI, $endI - $startI));
                        break;
                    }
                }
            }
