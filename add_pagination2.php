...
// 2. Extract applyFilters string position dynamically
$startSearch = "        // Filter\r\n        rows.forEach(row => {";
if (strpos($content, $startSearch) === false) {
    $startSearch = "        // Filter\n        rows.forEach(row => {";
}
...
if ($startPos !== false) {
    $endSearch = "        });\r\n\r\n        let emptyMsg";
    if (strpos($content, $endSearch, $startPos) === false) {
        $endSearch = "        });\n\n        let emptyMsg";
    }
    $endPos = strpos($content, $endSearch, $startPos);
    
    if ($endPos !== false) {
        // We want to replace everything from $startPos to $endPos + strlen("        });")
        $targetChunk = substr($content, $startPos, $endPos - $startPos + 11); // 11 is length of "        });"
        ...
        renderPagination();
EOF;
        $content = str_replace($targetChunk, $jsReplacement, $content);
        file_put_contents($file, $content);
        echo "Successfully injected pagination.\n";
    } else {
        echo "Could not find end of applyFilters.\n";
    }
} else {
    echo "Could not find start of applyFilters.\n";
}
